...
import { Bell, Plus, Search, CheckCircle2, AlertTriangle, Clock3, Trash2, Pencil } from 'lucide-react'
...
export default function FinanceFlow() {
  ...
  const [contas, setContas] = useState([
    ...
    {
      id: 8,
      nome: 'PLANO DE SAUDE NOTRE DAME',
      valor: 0,
      tipo: 'Mensal',
      criada: '2026-05-01',
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      const defaultContas = contas

      const merged = [
        ...defaultContas.filter(
          (defaultConta) =>
            !parsed.some((savedConta) => savedConta.nome === defaultConta.nome)
        ),
        ...parsed
      ]
  ...
  useEffect(() => {
    ...
    if (savedHistorico) {
      setHistoricoPagamentos(JSON.parse(savedHistorico))
    }
  }, [])

  useEffect(() => {
    localStorage.setItem(
      'financeflow-historico',
      JSON.stringify(historicoPagamentos)
    )
  }, [historicoPagamentos])
  ...
  const urgentAccounts = contas.filter(
    (c) => getPriority(c).label === 'URGENTE'
  )

  const nextAccounts = contas.filter(
    (c) => getPriority(c).label === 'PRÓXIMO'
  )

  const paidAccounts = contas.filter((c) => c.status === 'Pago')

  const totalMonth = contas.reduce((acc, conta) => acc + conta.valor, 0)
          ...
          <div className="p-8 border-b border-zinc-200 flex items-center justify-between">
            <div>
              ...
              <p className="text-zinc-500 mt-2">
                Prioridade automática baseada no vencimento
              </p>
            </div>
          ...
          <div className="p-8 space-y-5">
            {filteredContas.map((conta) => {
              ...
              return (
                <div
                  key={conta.id}
                  className="bg-white/70 border border-white/50 rounded-3xl p-6 flex flex-col lg:flex-row lg:items-center justify-between shadow-xl hover:scale-[1.01] transition-all"
                >
                  <div>
                    <div className="flex flex-wrap items-center gap-3">
                      <h2 className="text-3xl font-bold">{conta.nome}</h2>

                      <span className="bg-zinc-100 px-4 py-1 rounded-full text-sm font-medium">
                        {conta.tipo}
                      </span>
                    ...
                    <div className="flex flex-wrap gap-8 mt-5 text-zinc-600 text-lg">
                      <p>
                        💰 Valor: <strong>{formatMoney(conta.valor)}</strong>
                      </p>
                  ...
                  <div className="flex items-center gap-4 mt-6 lg:mt-0">
                    ...
                    <button
                      ...
                      className="bg-blue-500 text-white px-5 py-4 rounded-2xl font-semibold"
                    >
                      Histórico
                    </button>
            ...
            <div className="bg-white w-full max-w-3xl rounded-[35px] shadow-2xl p-8 max-h-[80vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-8">
                ...
                <button
                  ...
                  className="bg-zinc-200 px-5 py-3 rounded-2xl font-semibold"
                >
                  Fechar
                </button>
              </div>
              ...
              <div className="space-y-4">
                ...
                {(historicoPagamentos[showHistorico] || [])
                  ...
                  .map((registro) => (
                    <div
                      key={registro.id}
                      className="bg-white border border-zinc-200 rounded-3xl p-6 flex items-center justify-between shadow-lg"
                    >
                      <div>
                        ...
                        <p className="text-zinc-500 mt-2 text-lg">
                          Pagamento realizado em {registro.data}
                        </p>
                      </div>
            ...
            <div className="bg-white w-full max-w-2xl rounded-[35px] shadow-2xl p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <Input
                  ...
                  value={form.criada}
                  onChange={(e) =>
                    setForm({ ...form, criada: e.target.value })
                  }
                />
              ...
              <div className="flex justify-end gap-4 mt-10">
                <button
                  ...
                  className="bg-zinc-200 px-6 py-4 rounded-2xl font-semibold"
                >
                  Cancelar
                </button>
  ...
  return (
    <div className="bg-white/70 backdrop-blur-2xl border border-white/40 rounded-[32px] p-7 shadow-[0_10px_40px_rgba(0,0,0,0.10)] overflow-hidden min-h-[180px] flex flex-col justify-between hover:scale-[1.02] transition-all duration-300">
      ...
      <div className="mt-6 overflow-hidden">
        <h2 className="text-[42px] leading-none font-black tracking-[-2px] text-zinc-900 truncate">
          {value}
        </h2>
      </div>
    </div>
...
O .exe será criado automaticamente em:
...
✔️ Contas recorrentes
✔️ Persistência local
✔️ Alertas automáticos
✔️ Prioridade inteligente
